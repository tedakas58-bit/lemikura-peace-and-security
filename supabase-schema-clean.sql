-- Clean Supabase Database Schema for Lemi Kura Peace and Security Website
...
-- Create question_config table (only if it doesn't exist)
...
DROP POLICY IF EXISTS "Allow all operations on question_config" ON question_config;
...
-- Insert default question configuration (only if table is empty)
INSERT INTO question_config (config) 
SELECT '{
    "personal": [
        {
            "id": "fullName",
...
WHERE NOT EXISTS (SELECT 1 FROM question_config);

-- Insert sample news data (only if table is empty)
INSERT INTO news (title, category, excerpt, content, likes, date_display) 
SELECT * FROM (VALUES 
    (
...
) AS sample_data(title, category, excerpt, content, likes, date_display)
...
DROP TRIGGER IF EXISTS update_question_config_updated_at ON question_config;
