-- Customer Satisfaction Survey Database Schema for Supabase (Safe Version)
-- This version can be run multiple times without errors

-- Enable UUID extension
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Create survey_responses table (safe)
...
CREATE INDEX IF NOT EXISTS idx_survey_responses_education ON survey_responses(education_level);
CREATE INDEX IF NOT EXISTS idx_survey_responses_overall_score ON survey_responses(overall_score);
...
-- Insert default questions (safe - only if they don't exist)
INSERT INTO questions (question_id, dimension, text_amharic, text_english, order_number) 
SELECT * FROM (VALUES
    -- Tangibility
...
) AS new_questions(question_id, dimension, text_amharic, text_english, order_number)
WHERE NOT EXISTS (
    SELECT 1 FROM questions WHERE questions.question_id = new_questions.question_id
);
...
-- Create a view for analytics (safe)
